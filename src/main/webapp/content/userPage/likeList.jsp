...
<%
	MemberDTO likelist_sdto = (MemberDTO) session.getAttribute("loginUser");
/*********** url 정보 저장, 데이터 불러오기용 변수 작업 ***********/
	...
	ArrayList<MyPagePostDTO> list = dao.myLikePost(likelist_sdto.getMember_id(), startRow, endRow);
	
	int count = dao.myLikePostCount(likelist_sdto.getMember_id()); //  총 좋아요 글 수
...
<%		}%>
	...
	%>			<a href="myPage.jsp?page=like&pageNo=<%=startPage-pageBlock%>&pageLineSize=<%=pageLineSize%>">이전</a>
	<%		}
			
			// 현재 페이지 번호 버튼 [1][2][3] ... [10]
			for(int i = startPage; i <= endPage; i++){
	%>			<a href="myPage.jsp?page=like&pageNo=<%=i%>&pageLineSize=<%=pageLineSize%>"><%=i %></a>
	<%		}
			
			// [다음] 버튼		: 다음 블록이 있을때만 표시
			if( endPage < pageCount ){
	%>		<a href="myPage.jsp?page=like&pageNo=<%= startPage + pageBlock %>&pageLineSize=<%=pageLineSize%>">다음</a>
	<%		}
		}
	%>	
	</div>
